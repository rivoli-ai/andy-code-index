...
public interface IGitService
{
    ...
    Task<List<GitCommitInfo>> GetCommitsAsync(string repoDir, string gitRef, int limit = 50, string? beforeSha = null, CancellationToken ct = default);
    ...
    Task<byte[]?> ReadFileBytesAsync(string repoDir, string commitSha, string filePath, CancellationToken ct = default);
    ...
    Task<List<GitTreeEntry>> ListTreeAsync(string repoDir, string commitSha, string? path = null, bool recursive = false, CancellationToken ct = default);
    ...
    Task<string> GetHeadRefAsync(string repoDir, CancellationToken ct = default);
...
public class GitTreeEntry
{
    public required string Path { get; set; }
    public required string Name { get; set; }
    public required string Type { get; set; } // "blob" or "tree"
    public string? Hash { get; set; }
    public long Size { get; set; }
    public string? Language { get; set; }
}
